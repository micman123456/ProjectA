...
int32_t GenerateStairTile(TILE* Background_Tiles) {
    if (Room_Vector.empty()) {
        std::cerr << "Error: Room_Vector is empty. Cannot generate stair tile." << std::endl;
...
VOID ResetTiles(TILE *Background_Tiles, TILE* Tile_Type_Array){
    ...
    Background_Tiles[i].type = WALL_FULL;
   }
...
VOID SetTilesOverworld(TILE *Background_Tiles,GAMEBITMAP* FloorPlan)
{
    ...
    for(int PixelY = 0; PixelY<GBM_Height; PixelY+=TILE_SIZE){
        for(int PixelX = 0; PixelX<GBM_Width; PixelX+=TILE_SIZE){
            ...
            // Reference the col map for the overworld //
            ...
            // Pink = South transition //
            else if(Pixel.red == 255 && Pixel.blue == 255 && Pixel.green == 0)
            {
                Background_Tiles[tile_index+2].type=TRANSITION_TILE_S;
            }
            // Blue = North transition //
            else if(Pixel.red == 0 && Pixel.blue == 255 && Pixel.green == 255)
            {
                Background_Tiles[tile_index+2].type=TRANSITION_TILE_N;
            }
            // Green = East transition //
             else if(Pixel.red == 0 && Pixel.blue == 0 && Pixel.green == 255)
            {
                Background_Tiles[tile_index+2].type=TRANSITION_TILE_E;
            }
            // Yellow = West transition //
             else if(Pixel.red == 255 && Pixel.blue == 0 && Pixel.green == 255)
            {
                Background_Tiles[tile_index+2].type=TRANSITION_TILE_W;
            }
...
VOID SetTilesTestingZone(TILE *Background_Tiles){

    for(int i = 0; i < NUMB_TILES; i++)
    {
        
        Background_Tiles[i].type=FLOOR1;
    }

    Background_Tiles[STARTING_TILE+5].type=WALL_FULL;
